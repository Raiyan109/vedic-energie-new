...
import { WashingLoads } from '../../constants';
...
const WashingNoOfLoads = (washingselectedLoads, isWashingOn, handleWashingSelect) => {
  return (
    <div>
        <Section>
                <select name="watt" id="watt"
                    value={washingselectedLoads}
                    disabled={!isWashingOn}
                    onChange={handleWashingSelect}
                    className='w-16 h-8 bg-lightGreen rounded-md flex justify-center items-center text-xl text-rgbaHeader select'>
                    {
                        WashingLoads.map((range, idx) => (
...
export default WashingNoOfLoads